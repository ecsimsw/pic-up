...
<img src="https://github.com/ecsimsw/pic-up/assets/46060746/3848d248-6a9c-416b-a82a-16ce5349ce97" width="700">

</br>

- 이미지는 리사이징, 동영상은 프레임을 캡처하여 썸네일 파일을 생성한다.
- 원본, 썸네일 파일을 동시 업로드한다. 모든 업로드가 성공했음을 확인 후, 사용자에게 응답한다.
- 업로드에 실패하는 경우, 사용자에게 그 즉시 예외를 알리되 진행 중인 업로드는 완료 후 제거 로직이 수행된다.
- 분산락으로 사용자별 DB 트랜잭션을 격리하고, 이미지 업로드 등 임계구역과 관계없는 로직을 락 범위에서 분리하여 동시성을 개선한다.
- 파일이 정상 업로드 되는 경우, 동영상 썸네일 제작을 위해 로컬 스토리지에 저장한 임시 파일을 비동기 제거한다.

#### 파일 다중 제거

<img src="https://github.com/ecsimsw/pic-up/assets/46060746/7c4ba472-c401-473d-9f22-9477f1b45411" width="700">

</br>

- 사진을 제거하면 DB 소프트딜리트 후 사용자 응답, 실제 파일 삭제는 비동기로 처리된다.
- 비동기 작업의 예외처리로 복잡해지는 코드, 비정상 종료로 작업이 유실되는 상황을 피하고 싶었다.
- 작업 실패, 타임아웃, Dead Letter 문제를 메시지 큐에서 처리하는 것으로 코드 복잡도를 낮추고 작업이 유실되는 경우를 방지한다.
- N개씩 순차적으로 처리하는 것으로, 한번에 많은 파일 삭제가 몰리는 상황을 대비한다.

</br>

<img src="https://github.com/ecsimsw/pic-up/assets/46060746/757b3766-79e9-43c6-abb7-093cb81dafa4" width="700">

- 삭제 요청 처리 중 예외가 발생하는 경우 DB는 롤백되지만 MQ에 이미 전송한 이벤트는 롤백되지 않고, DB 정보는 있지만 파일은 존재하지 않는 문제가 발생한다.
- 삭제 요청에 메시지를 전송하는 것이 아닌, 메시지 내용을 DB에 우선 저장하여 `@Transaction`를 사용
