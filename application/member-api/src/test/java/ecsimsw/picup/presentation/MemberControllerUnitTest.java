...
import static ecsimsw.picup.utils.MemberFixture.ACCESS_TOKEN;
import static ecsimsw.picup.utils.MemberFixture.REFRESH_TOKEN;
...
class MemberControllerUnitTest {
    ...
    private final MockMvc mockMvc = MockMvcBuilders
        .standaloneSetup(new MemberController(memberFacadeService, authTokenService))
    ...
    public void signInTest() throws Exception {
        ...
        var loginUser = new TokenPayload(memberResponse.id(), memberResponse.username());
    ...
    public void signInTestInvalidLoginInfo() throws Exception {
        ...
        when(memberFacadeService.signIn(signInInfo))
    ...
    public void signUp() throws Exception {
        ...
        var memberInfo = new MemberInfo(1L, signUpRequest.username());
        var loginUser = new TokenPayload(memberInfo.id(), memberInfo.username());
        var loginAuthToken = AuthTokens.of(loginUser, ACCESS_TOKEN, REFRESH_TOKEN);
        var expectMemberResponse = MemberResponse.of(memberInfo);

        when(memberFacadeService.signUp(signUpRequest))
            .thenReturn(expectMemberResponse);
        when(authTokenService.issue(loginUser))
            .thenReturn(loginAuthToken);
        when(authTokenService.accessTokenCookie(loginAuthToken))
            .thenReturn(new Cookie(ACCESS_TOKEN_COOKIE_NAME, ACCESS_TOKEN));
        ...
        mockMvc.perform(post("/api/member/signup")
            ...
            .andExpect(cookie().value(ACCESS_TOKEN_COOKIE_NAME, ACCESS_TOKEN))
            .andExpect(cookie().value(REFRESH_TOKEN_COOKIE_NAME, REFRESH_TOKEN))
            .andExpect(content().json(OBJECT_MAPPER.writeValueAsString(expectMemberResponse)));
    }
